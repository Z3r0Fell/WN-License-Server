...
Reads provider configuration from runtime_settings (DB-backed) with env
fallback so admins can change provider creds from the /admin/settings UI
...
def _from_addr() -> tuple[str, str]:
    addr = rs.get("EMAIL_FROM", "licenses@watchnexus.app")
    name = rs.get("EMAIL_FROM_NAME") or rs.get("EMAIL_FROM_BRAND") or "WatchNexus"
    return name, addr
...
def _send_sendgrid(from_addr: str, from_name: str, to: str, subject: str, html: str, text: str) -> dict:
    ...
    sg = SendGridAPIClient(rs.get("SENDGRID_API_KEY"))
    res = sg.send(msg)
...
def _send_smtp(from_addr: str, from_name: str, to: str, subject: str, html: str, text: str) -> dict:
    host = rs.get("SMTP_HOST")
    port = rs.get_int("SMTP_PORT", 587)
    user = rs.get("SMTP_USERNAME")
    pw = rs.get("SMTP_PASSWORD")
    use_tls = rs.get_bool("SMTP_USE_TLS", True)
    msg = EmailMessage()
...
def render_purchase_email(*, customer_email: str, license_key: str, product_name: str,
    ...
    brand = rs.get("EMAIL_FROM_BRAND") or rs.get("EMAIL_FROM_NAME") or "WatchNexus"
    html = f"""
<!doctype html>
<html><body style="font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,sans-serif;background:#0B1220;color:#E2E8F0;margin:0;padding:0;">
  <div style="max-width:560px;margin:0 auto;padding:32px 24px;">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:24px;">
      ...
      <div style="font-weight:600;letter-spacing:.02em;">{brand}</div>
    </div>
