...
| `failed to compute cache key: "/frontend/yarn.lock": not found` (Docker build) | `yarn.lock` wasn't committed to the repo. Quick fix: pull latest (the Dockerfile now tolerates a missing lockfile). Proper fix: on a machine that has `node_modules` built, run `cd frontend && yarn install` to regenerate `yarn.lock`, then `git add -f frontend/yarn.lock && git commit -m "add yarn.lock" && git push`. |
